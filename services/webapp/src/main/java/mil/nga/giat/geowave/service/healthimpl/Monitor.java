...
public class Monitor
{
	...
	private static class MaxList<T> extends
			LinkedList<Pair<Long, T>>
	{
	...
	private static final List<Pair<Long, Double>> loadOverTime = Collections.synchronizedList(new MaxList<Double>(
			MAX_TIME_PERIOD));
	private static final List<Pair<Long, Double>> ingestRateOverTime = Collections.synchronizedList(new MaxList<Double>(
			MAX_TIME_PERIOD));
	private static final List<Pair<Long, Double>> ingestByteRateOverTime = Collections.synchronizedList(new MaxList<Double>(
			MAX_TIME_PERIOD));
	private static final List<Pair<Long, Integer>> minorCompactionsOverTime = Collections.synchronizedList(new MaxList<Integer>(
			MAX_TIME_PERIOD));
	private static final List<Pair<Long, Integer>> majorCompactionsOverTime = Collections.synchronizedList(new MaxList<Integer>(
			MAX_TIME_PERIOD));
	private static final List<Pair<Long, Double>> lookupsOverTime = Collections.synchronizedList(new MaxList<Double>(
			MAX_TIME_PERIOD));
	private static final List<Pair<Long, Integer>> queryRateOverTime = Collections.synchronizedList(new MaxList<Integer>(
			MAX_TIME_PERIOD));
	private static final List<Pair<Long, Integer>> scanRateOverTime = Collections.synchronizedList(new MaxList<Integer>(
			MAX_TIME_PERIOD));
	private static final List<Pair<Long, Double>> queryByteRateOverTime = Collections.synchronizedList(new MaxList<Double>(
			MAX_TIME_PERIOD));
	private static final List<Pair<Long, Double>> indexCacheHitRateOverTime = Collections.synchronizedList(new MaxList<Double>(
			MAX_TIME_PERIOD));
	...
	private static class EventCounter
	{
		...
		void updateTabletServer(
				String name,
				long sampleTime,
				long numEvents ) {
			Pair<Long, Long> newSample = new Pair<Long, Long>(
					sampleTime,
					numEvents);
		...
		void finishedUpdating() {
			...
			samples.keySet().retainAll(
					serversUpdated);
			prevSamples.keySet().retainAll(
					serversUpdated);
		}
		...
		try {
			...
			if (mmi != null) {
				...
				for (TabletServerStatus server : mmi.tServerInfo) {
					...
					lookupRateTracker.updateTabletServer(
							server.name,
							server.lastContact,
							server.lookups);
				...
				ingestRateOverTime.add(new Pair<Long, Double>(
						currentTime,
						totalIngestRate));
				ingestByteRateOverTime.add(new Pair<Long, Double>(
						currentTime,
						totalIngestByteRate));
				...
				loadOverTime.add(new Pair<Long, Double>(
						currentTime,
						totalLoad));
				...
				lookupsOverTime.add(new Pair<Long, Double>(
						currentTime,
						lookupRateTracker.calculateRate()));

				queryRateOverTime.add(new Pair<Long, Integer>(
						currentTime,
						(int) totalQueryRate));
				queryByteRateOverTime.add(new Pair<Long, Double>(
						currentTime,
						totalQueryByteRate));

				scanRateOverTime.add(new Pair<Long, Integer>(
						currentTime,
						(int) totalScanRate));

				calcCacheHitRate(
						indexCacheHitRateOverTime,
						currentTime,
						indexCacheHitTracker,
						indexCacheRequestTracker);
				calcCacheHitRate(
						dataCacheHitRateOverTime,
						currentTime,
						dataCacheHitTracker,
	...
	private static void calcCacheHitRate(
		...
		if (req > 0)
			hitRate.add(new Pair<Long, Double>(
					currentTime,
					cacheHits.calculateCount() / (double) cacheReq.calculateCount()));
		else
			hitRate.add(new Pair<Long, Double>(
					currentTime,
					null));
	...
	private static GCStatus fetchGcStatus() {
		...
		try {
			...
			List<String> locks = zk.getChildren(
					path,
					null);
			if (locks != null && locks.size() > 0) {
				Collections.sort(locks);
				address = new ServerServices(
						new String(
								zk.getData(
										path + "/" + locks.get(0),
										null),
				...
				GCMonitorService.Client client = ThriftUtil.getClient(
						new GCMonitorService.Client.Factory(),
						address,
						new AccumuloServerContext(
								config));
				try {
			...
			log.warn(
					"Unable to contact the garbage collector at " + address,
					ex);
		}
		return result;
	}
	...
	public void run(
		...
		int port = config.getConfiguration().getPort(
		...
		try {
			...
			log.error(
					"Unable to start embedded web server",
					ex);
		...
		try {
			...
			log.error(
					"Unable to set monitor HTTP address in zookeeper",
					ex);
		}
	...
	public static Map<HostAndPort, ScanStats> getScans() {
		synchronized (allScans) {
			return new HashMap<HostAndPort, ScanStats>(
					allScans);
		}
	}
	...
	protected static void fetchScans()
		...
		for (String server : c.instanceOperations().getTabletServers()) {
			...
			Client tserver;
			try {
				tserver = ThriftUtil.getTServerClient(
						parsedServer,
						context);
			}
			catch (Exception e) {
			...
			try {
				List<ActiveScan> scans = tserver.getActiveScans(
						null,
						context.rpcCreds());
				synchronized (allScans) {
					allScans.put(
				...
				log.debug(
						"Failed to get active scans from {}",
						server,
						ex);
			}
			finally {
	...
	private void getMonitorLock()
		...
		while (true) {
			...
			monitorLock = new ZooLock(
					monitorLockPath);
			monitorLock.lockAsync(
					monitorLockWatcher,
					new byte[0]);
			 ...
			 * 
			 * 
			 * if (!monitorLockWatcher.failedToAcquireLock) { throw new
			 * IllegalStateException( "monitor lock in unknown state"); }
			 */

			monitorLock.tryToCancelAsyncLockOrUnlock();

			UtilWaitThread.sleep(getContext().getConfiguration().getTimeInMillis(
					Property.MONITOR_LOCK_CHECK_INTERVAL));
		}
			...
			ZooLock.AsyncLockWatcher
	{
		...
		public void lostLock(
			...
			Halt.halt(
					"Monitor lock in zookeeper lost (reason = " + reason + "), exiting!",
					-1);
		}

		@Override
		public void unableToMonitorLockNode(
				final Throwable e ) {
			Halt.halt(
					-1,
					new Runnable() {
						...
						public void run() {
							log.error(
									"No longer able to monitor Monitor lock node",
									e);
						}
					});
		...
		public synchronized void acquiredLock() {
			if (acquiredLock || failedToAcquireLock) {
				Halt.halt(
						"Zoolock in unexpected state AL " + acquiredLock + " " + failedToAcquireLock,
						-1);
			}
		...
		@Override
		public synchronized void failedToAcquireLock(
				Exception e ) {
			log.warn("Failed to get monitor lock " + e);

			if (acquiredLock) {
				Halt.halt(
						"Zoolock in unexpected state FAL " + acquiredLock + " " + failedToAcquireLock,
						-1);
			}
