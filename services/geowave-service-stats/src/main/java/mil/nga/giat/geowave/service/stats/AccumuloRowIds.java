 ...
 * 
 */
public class AccumuloRowIds {
	private final byte[] insertionId;
	private final byte[] dataId;
	private final byte[] adapterId;
	private final int numberOfDuplicates;

	public AccumuloRowIds(final Key key) {
		this(key.getRow().copyBytes());
	}

	public AccumuloRowIds(final byte[] accumuloRowId) {
		final byte[] metadata = Arrays.copyOfRange(accumuloRowId,
				accumuloRowId.length - 12, accumuloRowId.length);
		System.out.println(accumuloRowId.length);
		
		final ByteBuffer metadataBuf = ByteBuffer.wrap(metadata);
		final int adapterIdLength = metadataBuf.getInt();
		
		System.out.println("int = "+adapterIdLength);
		final int dataIdLength = metadataBuf.getInt();
		final int numberOfDuplicates = metadataBuf.getInt();

		final ByteBuffer buf = ByteBuffer.wrap(accumuloRowId, 0,
				accumuloRowId.length - 12);
		final byte[] insertionId = new byte[accumuloRowId.length - 12
				- adapterIdLength - dataIdLength];
	...
	public AccumuloRowIds(final byte[] indexId, final byte[] dataId,
			final byte[] adapterId, final int numberOfDuplicates) {
	...
	public byte[] getRowId() {
		final ByteBuffer buf = ByteBuffer.allocate(12 + dataId.length
				+ adapterId.length + insertionId.length);
		buf.put(insertionId);
		buf.put(adapterId);
		buf.put(dataId);
